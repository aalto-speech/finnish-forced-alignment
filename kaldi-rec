...
def Aalto_speaker_segment_audio(SOX_PATH,srate,infile):
    ...
    diarization_file = open(diarization_filename,"r", encoding="utf-8")
...
def audio_file_len(audio_filename):
    ms = 0
    audiofile = open(audio_filename,"r", encoding="utf-8")
    audiofile.seek(28)
    a=audiofile.read(4)
...
def create_data_folder(target_wav_filename,seg_filenames):
    ...
    wav_scp_file = open(wav_scp_filename,"w", encoding="utf-8")
    audio_filenames = []
    ...
    wav_list_file = open(wav_list_filename,"w", encoding="utf-8")
    ...
    verbatim_file = open(verbatim_filename,"w", encoding="utf-8")
    for audio_filename in audio_filenames:
        #file_id,ext= audio_filename.rsplit(".",1)
        #ext,file_id = file_id.rsplit("/",1)
        #file_id = file_id.strip()
    ...
    spk2utt_file = open(spk2utt_filename,"w", encoding="utf-8")
    for speaker,utt in spk2utt.items():
        spk2utt_file.write(speaker+" "+utt.strip()+"\n")
    spk2utt_file.close()
    
    #utt2spk
    utt2spk_filename = kaldi_data_dir+"/utt2spk"
    utt2spk_file = open(utt2spk_filename,"w", encoding="utf-8")
...
def write_srt(asr_segmentations,srt_filename):
    ...
    srt_file = open(srt_filename,"w", encoding="utf-8")
...
asr_models = {
    'fi': { 'am-model': 'models/fi/all-lstm',
             ...
             'decoding-graph':'models/fi/all-lstm/aalto-speech',
             'decode-params':"--extra-left-context 40 --extra-right-context 0 --frames-per-chunk 140 --beam 20 --lattice-beam 10.0 --min-active 12000 --skip-scoring false --num-threads 4 --cmd 'run.pl --mem 8G' --post-decode-acwt 10.0 --acwt 1.0 --scoring-opts \"--min-lmwt 4 --max-lmwt 18 --decode_mbr true\" --online-ivector-dir",
...
def main():    
    ...
    # FIXME: This should be fixed by modifying a local copy of decode.sh
    # link model, opts
    os.system("ln -sf "+AM_MODEL+"/cmvn_opts  "+data_folder_path)
    os.system("ln -sf "+AM_MODEL+"/final.mdl  "+data_folder_path)
  
    #Recognize 
    decoding_output_path = data_folder_path+"/decode_output"
    print(rootdir+"steps/nnet3/decode.sh --nj "+str(number_of_speakers)+" "+decode_params+" "+ivectors_dir+" "+DECODING_GRAPH+" "+data_folder_path+" "+decoding_output_path)
    ...
    words_dict = dict()
    for line in fileinput.input(words_filename, openhook=fileinput.hook_encoded("utf-8")):
        line = line.strip()
    ...
    sub_word_start = ""
    sub_word_end = ""
    for line in fileinput.input(ctm_filename, openhook=fileinput.hook_encoded("utf-8")):
        line = line.strip()

...
def split_silence(created_df, split_type, pause_duration):
    ...
    for i in range(len(start_list_ms)):
        pause = end_list_ms[i] < start_list_ms[i + 1]
...
def main(gold_ctms_file, created_ctms_file, split_type, pause_duration):
    _, created_ctm_df = create_ctm_dfs(gold_ctms_file, created_ctms_file)
    split_silence(created_ctm_df, split_type, pause_duration)


if __name__ == '__main__':
    arguments = parse_arguments()
    main(arguments.gold, arguments.created, arguments.split_type, arguments.pause_duration)
